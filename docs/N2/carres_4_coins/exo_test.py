...
for X, Y, A, N in [
    ...
    for (a, k, x, y), (a_, k_, x_, y_) in zip(psolver, psetter):
        ...
        if not sont_proches(a, a_):
            if a < a_:
                assert False, f"Pour (x, y, a, n) = {(X, Y, A, N)}, il manque un carré de taille {a_}"
            else:
                assert False, f"Pour (x, y, a, n) = {(X, Y, A, N)}, il a un carré de taille {a_} en trop"
        if not (sont_proches(x, x_) and sont_proches(y, y_)):
            assert False, f"Pour (x, y, a, n) = {(X, Y, A, N)}, il manque un carré de taille {a_} en ({x_}, {y_})"
