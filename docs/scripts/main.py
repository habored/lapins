def define_env(env):
    ...
    def create_unittest_button(
        tc: str, nom_script: str, path: str, mode: str, MAX: int = 5
    ) -> str:
        """
    ...
    def get_max_from_file(
        content: str,
    ) -> tuple:  # [str, int]: # compatibilité Python antérieur 3.8
    ...
    def test_style(nom_script: str, element: str) -> bool:
        ...
        ide_style = ["", "v"]
        styles = [
            f"""IDE{istyle}({i}{nom_script}{i}"""
            for i in guillemets
            for istyle in ide_style
        ]
    ...
    def IDE(nom_script: str = "", mode: str = "h", MAX: int = 5, SANS: str = "") -> str:
        ...
        corr_content, _ = generate_content(
            f"""{'/'.join(nom_script.split('/')[:-1])}/{nom_script.split('/')[-1]}_corr""",
            path_file,
        )
        ...
        div_edit += tooltip_button(
            ...
            f"""<img src="/{path_img}/images/buttons/icons8-play-64.png"><span class="tooltiptext">Lancer</span>""",
        )
        div_edit += create_unittest_button(
            tc, nom_script, path_file, mode, MAX
        ) + blank_space(1)
        div_edit += (
            tooltip_button(
                f"""\'downloadFile("editor_{tc}","{nom_script}")\'""",
                f"""<img src="/{path_img}/images/buttons/icons8-download-64.png"><span class="tooltiptext">Télécharger</span>""",
            )
            + blank_space()
        )
        ...
        div_edit += (
            tooltip_button(
                f"""\'reload("{tc}")\'""",
                f"""<img src="/{path_img}/images/buttons/icons8-restart-64.png"><span class="tooltiptext">Recharger</span>""",
            )
            + blank_space()
        )
        div_edit += tooltip_button(
            f"""\'saveEditor("{tc}")\'""",
            f"""<img src="/{path_img}/images/buttons/icons8-save-64.png"><span class="tooltiptext">Sauvegarder</span>""",
        )
        div_edit += "</div>"
        ...
        if nom_script == "":
            indent = " "  # to avoid conflict with empty IDEs
        if indent == "":
            div_edit += f"""
